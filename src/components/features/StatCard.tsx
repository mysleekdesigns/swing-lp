...
export function StatCard({ label, value, index }: StatCardProps) {
  return (
    ...
    >
      ...
      {/* Glass morphism card */}
      <div className="relative h-full bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/20 shadow-xl">
        {/* Value */}
        <div className="text-5xl md:text-6xl font-bold mb-3 bg-gradient-to-r from-pink-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
          <AnimatedCounter value={value} duration={2.5} />
        </div>

        {/* Label */}
        <div className="text-gray-300 text-lg font-medium leading-tight">
          {label}
        </div>
